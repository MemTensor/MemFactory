#!/bin/bash
set -x

DATA_PATH="./datas/converted_hotpotqa_2000.json"
MODEL_NAME="Qwen3-4B-Instruct"
MODEL_ROOT="${MODEL_ROOT:-./models}"
MODEL_PATH="${MODEL_ROOT}/${MODEL_NAME}"
if [ ! -d "$MODEL_PATH" ] && [ -d "/mnt/afs/models/${MODEL_NAME}" ]; then
    MODEL_PATH="/mnt/afs/models/${MODEL_NAME}"
fi
OUTPUT_DIR="./output/NoMemoryGRPO4B_DDP"

ENV_TYPE="longcontext"
AGENT_TYPE="no_memory"

NPROC_PER_NODE="${NPROC_PER_NODE:-4}"
LR="${LR:-5e-6}"
BETA="${BETA:-0.001}"
NUM_GENS="${NUM_GENS:-16}"
MAX_PROMPT_LEN="${MAX_PROMPT_LEN:-12000}"
MAX_GEN_LEN="${MAX_GEN_LEN:-1500}"
GRAD_ACC_STEPS="${GRAD_ACC_STEPS:-6}"
SAVE_STEPS="${SAVE_STEPS:-250}"
MAX_STEPS="${MAX_STEPS:-250}"
GENERATION_BATCH_SIZE="${GENERATION_BATCH_SIZE:-1}"
TRAIN_MICRO_BATCH_SIZE="${TRAIN_MICRO_BATCH_SIZE:-1}"
LOGPROB_BATCH_SIZE="${LOGPROB_BATCH_SIZE:-1}"
CONTEXT_TRUNCATION_SIDE="${CONTEXT_TRUNCATION_SIDE:-head}"
DDP_TIMEOUT_SECONDS="${DDP_TIMEOUT_SECONDS:-1800}"
LLM_REWARD_MAX_WORKERS="${LLM_REWARD_MAX_WORKERS:-8}"
OPENAI_TIMEOUT_SECONDS="${OPENAI_TIMEOUT_SECONDS:-300}"

export DDP_TIMEOUT_SECONDS
export LLM_REWARD_MAX_WORKERS
export OPENAI_TIMEOUT_SECONDS
export TORCH_NCCL_ASYNC_ERROR_HANDLING="${TORCH_NCCL_ASYNC_ERROR_HANDLING:-1}"
export TORCH_NCCL_TRACE_BUFFER_SIZE="${TORCH_NCCL_TRACE_BUFFER_SIZE:-2000}"
export TORCH_NCCL_DUMP_ON_TIMEOUT="${TORCH_NCCL_DUMP_ON_TIMEOUT:-1}"
export NCCL_DEBUG="${NCCL_DEBUG:-WARN}"

cd "$(dirname "$0")/.."

echo "Starting no-memory GRPO DDP baseline..."
echo "Model: $MODEL_NAME"
echo "Model path: $MODEL_PATH"
echo "NPROC_PER_NODE: $NPROC_PER_NODE"
echo "Env: $ENV_TYPE | Agent: $AGENT_TYPE"
echo "DDP timeout seconds: $DDP_TIMEOUT_SECONDS"
echo "LLM reward workers per rank: $LLM_REWARD_MAX_WORKERS"

torchrun --standalone --nproc_per_node "$NPROC_PER_NODE" examples/train_mem_grpo.py \
    --model_name_or_path "$MODEL_PATH" \
    --data_path "$DATA_PATH" \
    --output_dir "$OUTPUT_DIR" \
    --lr "$LR" \
    --beta "$BETA" \
    --gradient_accumulation_steps "$GRAD_ACC_STEPS" \
    --train_micro_batch_size "$TRAIN_MICRO_BATCH_SIZE" \
    --logprob_batch_size "$LOGPROB_BATCH_SIZE" \
    --num_generations "$NUM_GENS" \
    --generation_batch_size "$GENERATION_BATCH_SIZE" \
    --chunk_size 2500 \
    --max_chunk_number 6 \
    --max_prompt_length "$MAX_PROMPT_LEN" \
    --max_generate_length "$MAX_GEN_LEN" \
    --context_truncation_side "$CONTEXT_TRUNCATION_SIDE" \
    --save_steps "$SAVE_STEPS" \
    --max_steps "$MAX_STEPS" \
    --env_type "$ENV_TYPE" \
    --agent_type "$AGENT_TYPE" \
    --wandb_name "memfactory_${AGENT_TYPE}_${ENV_TYPE}_${MODEL_NAME}_ddp${NPROC_PER_NODE}" \
    --epoch 2
